...
include "header_in.php";
	...
	$id=$_SESSION["wp_id"];
	$query = "SELECT * FROM works WHERE wp_id='$id'";
	...
	while($row = mysqli_fetch_assoc($result))
{
	...
	while($row2= mysqli_fetch_assoc($result2)){
		...
		<b>{$w_id}</b><p><span>{$w_name}</span>  For {$wt_name}</p><p><span>{$shift} Shift work </span>vacancy : {$vac} / {$req}</p>
  </div></a>";
